...
export default function Servers() {
  const { identity, servers: memberships, call, setActiveServer, refreshServers } = useSession()
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => { createPanel.current?.scrollIntoView?.({ block: 'start' }); nameInput.current?.focus() })
  }, [searchParams])
  async function enter(server: Server) {
    setNotice('')
    try {
      const joined = memberships.some(membership => membership.id === server.id)
      if (!joined && server.join_policy === 'public') { await call('JoinServer', { server_id: server.id }); await refreshServers() }
      else if (joined) { /* Existing memberships open without another join request. */ }
      ...
      else { setNotice(`${server.name} requires an invitation before it can be opened.`); return }
  ...
  async function create(event: FormEvent) {
    ...
    try { const server = await call<Server>('CreateServer', { name, description, purpose: description, join_policy: policy, discoverable: true, tags: ['collaboration'] }); setActiveServer(server); await Promise.all([refresh(), refreshServers()]); navigate(`/servers/${server.id}/overview`) } catch (reason) { setNotice(messageForError(reason)) }
  }
