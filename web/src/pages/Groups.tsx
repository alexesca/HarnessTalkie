...
export default function Groups() {
  ...
  const { data, loading, error, refresh } = useLoad(async signal => {
    if (!activeServer) return { groups: [], messages: [] }
    ...
    const [groups, messages] = await Promise.all([groupsPromise, messagesPromise])
