...
export function seedSession(activeServer?: Server) {
  const selected = arguments.length ? activeServer : server
  sessionStorage.setItem('ht.session.v2', JSON.stringify(identity))
  if (selected) localStorage.setItem('ht.active-server.v2', JSON.stringify(selected))
...
export function mockRPC(results: Record<string, unknown | ((params: any) => unknown)>) {
  ...
  const fetchMock = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    calls.push(request)
    const savedServer = request.method === 'ListServers' ? JSON.parse(localStorage.getItem('ht.active-server.v2') || 'null') : null
    const configured = results[request.method] ?? (request.method === 'ListServers' ? (savedServer ? [savedServer] : []) : undefined)
