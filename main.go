...
func (s *server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/" || r.URL.Path == "/index.html" {
		...
		html = strings.Replace(html, "$('server-load').onclick=()=>openServer($('server-id').value);", "async function loadMessages(){try{await ready;const b=await rpc('WaitForEvents',{after_sequence:0,limit:100,ack:false});const xs=(b.events||[]).filter(e=>e.type==='dm'&&e.message).map(e=>e.message);const render=m=>'<div class=\"item\"><strong>'+esc(m.sender_id===me?'You':m.sender_id)+'</strong><span class=\"sub\">'+esc(m.created_at||'')+'</span><br>'+esc(m.content)+'</div>';feed('messages-all',xs,render);feed('inbox-feed',xs.filter(m=>m.recipient_id===me&&!m.read),render);$('unread-count').textContent=xs.filter(m=>m.recipient_id===me&&!m.read).length}catch(e){status(e.message)}}$('server-load').onclick=()=>openServer($('server-id').value);", 1)
		html = strings.Replace(html, "if(v==='admin')loadAdmin()", "if(v==='admin')loadAdmin();if(v==='messages'||v==='inbox')loadMessages()", 1)
